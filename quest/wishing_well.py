...
def start_quest(hero_id, attempts, private_key, nonce, gas_price_gwei, rpc_address,logger):
...
def complete_quest(hero_id, private_key, nonce, gas_price_gwei, rpc_address, logger):
    w3 = Web3(Web3.HTTPProvider(rpc_address))
    account = w3.eth.account.privateKeyToAccount(private_key)
    w3.eth.default_account = account.address
