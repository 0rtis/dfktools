...
def incubate_egg(egg_type, tier, private_key, nonce, gas_price_gwei, tx_timeout_seconds, rpc_address, logger):
    ...
    tx = contract.functions.incubateEgg(egg_type, tier).buildTransaction({'gasPrice': w3.toWei(gas_price_gwei, 'gwei'), 'nonce': nonce})
...
def crack(egg_id, private_key, nonce, gas_price_gwei, tx_timeout_seconds, rpc_address, logger):
    ...
    tx = contract.functions.crack(egg_id).buildTransaction({'gasPrice': w3.toWei(gas_price_gwei, 'gwei'), 'nonce': nonce})
